...
function getListsUsers(twit, lists, user, functionCallback){
	...
	if (lists ==  null || lists.length == 0){
		console.log("TURACO_DEBUG - USER's List Array Empty");
		var set = {};
		set.usersListHash = {};
	...
	function checkArrayPossition(array){
		x++;
		if(x < length) {
			console.log("TURACO_DEBUG - Loading list no: " + x);
	...
	function executeProcess(){
		...
		async.waterfall([
		                 ...
		                 function(_savedSearches, callback){
		                	 console.log("TURACO_DEBUG - " + _method + " getSavedSearches result");
		                	 global.usersInProgress[user.uid] = {
	             				completed : false,
	             				percent : 10
		                	 };
		                 ...
		                 function(data, callback){
		                	 console.log("TURACO_DEBUG - " + _method + " getUsersListFunction result");
		                	 global.usersInProgress[user.uid] = {
	             				completed : false,
	             				percent : 20
		                	 };
		                 ...
		                 function(set, callback){
		                	 console.log("TURACO_DEBUG - " + _method + " getListsUsers result");
		                	 if (!set) { callback(err); }
		                 ...
		                 function(dataAllFriends, callback){
							...
							console.log("TURACO_DEBUG - " + _method + " waterfall final method " +
							...
							if (err){ 
								...
								ListInfoTemporal.remove({uid: user.uid}, function(err){
									if (err) console.log("TURACO_DEBUG - Error deleting items: " +  err);
								});
								
								//remove all generated the info 
